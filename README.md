...
WeatherStar 4000+ can rotate through one or more embedded video panels. Each panel keeps the classic header chrome and uses the configured _title_ and _duration_ when it appears in the playlist. Supported sources include:

- **YouTube** links (`youtube.com/watch`, `youtu.be`, or `youtube-nocookie.com`) – rendered with the privacy-enhanced embed and the IFrame Player API
- **HLS** streams (`.m3u8`) – played natively when supported or via a lazily loaded copy of [Hls.js](https://github.com/video-dev/hls.js)
- **Generic iframes** – any embeddable URL that allows framing

Open the **Settings → Video** section to add, remove, and reorder sources. Each source supports the following options:

- _Title_ (displayed in the WeatherStar header)
- _URL_ (the stream or embed)
- _Duration_ in seconds (minimum 5s)
- _Type_ override (auto-detects when left on “Auto”)
- _Muted_ and _Loop_ toggles
- _Preload lead time_ (`preloadSec`) for fine-tuning how aggressively the player is warmed up before show time
- Advanced fields for poster art plus optional `startAt` / `endAt` offsets

Settings are written to local storage under `ws.video.sources`. To share a configuration, enable **Include videos in permalink** in the Video settings panel. The generated URL appends one `video=` parameter per source containing a URL-safe base64 payload with `title`, `url`, and `durationSec` (plus any non-default options). Example:

```
https://weatherstar.netbymatt.com/?video=eyJ0IjoiRG93bnRvd24gQ2FtIiwidSI6Imh0dHBzOi8vd3d3LnlvdXR1YmUuY29tL3dhdGNoP3Y9ZEVSRm5oN09PIiwiZCI6MjB9
```

When using environment variables with the server or Docker images, add numbered keys that mirror the expanded parameters. For example:

```
WSQS_video_0_title="Traffic Cam – Downtown"
WSQS_video_0_url="https://www.youtube.com/watch?v=dERFnh7OOO"
WSQS_video_0_dur=20
WSQS_video_0_type=auto
WSQS_video_1_url="https://example.com/live/stream.m3u8"
```

Environment keys convert underscores to dashes, so the example above emits `video-0-title`, `video-0-url`, etc. You can also supply the compact `video=` form directly (for single sources) with `WSQS_video=`.

Videos default to muted playback to satisfy autoplay policies. Audio can be enabled on a per-source basis, but browsers may still require a user gesture. All players are pre-warmed off-screen so that playback begins immediately when the panel becomes visible. If a source fails repeatedly, a “Video unavailable” message is shown and the slot continues advancing with the configured cadence.
